...
class MM_Model(nn.Module):
    def __init__(self, n_users, n_items, embedding_dim, weight_size,
                 dropout_list, image_feats, text_feats, user_init_embedding,
                 item_attribute_dict, movies_entities_embedding):
        ...
        ## BGU edit
        movies_entities_embedding_ = [ins_lst for out_lst in movies_entities_embedding for ins_lst in out_lst]
        
        for idx, vector in enumerate(movies_entities_embedding_) :
            if type(vector) == int:
                movies_entities_embedding_[idx] = torch.empty(1500)
                
        self.movie_entity_trans = nn.Linear(movies_entities_embedding_[0].shape[0], args.embed_size)  
        ...
        ## BGU edit
        nn.init.xavier_uniform_(self.movie_entity_trans.weight)
        ...
        self.movies_entities_embedding = movies_entities_embedding_
    ...
    def forward(self, ui_graph, iu_graph, image_ui_graph, image_iu_graph, text_ui_graph, text_iu_graph):
        ...
        movies_entities_embedding_on_device = [tensor.to(device) for tensor in self.movies_entities_embedding]
        movies_entities_embedding_agg = torch.stack(movies_entities_embedding_on_device)
        movies_entities_embedding_trans = self.movie_entity_trans(movies_entities_embedding_agg)
        # movie_entityes_feats = self.dropout(movies_entities_embedding_trans)
        self.item_feats['entity'] = self.dropout(movies_entities_embedding_trans)
        ##
        ...
        for key in self.item_feats.keys():
            ...
            user_feat_from_item[key] = self.mm(ui_graph, item_feats[key]) # Multiplying the movies user matrix with the representation of movie features
