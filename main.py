...
class Trainer(object):
    def __init__(self, data_config):
        ...
        self.ui_graph = self.ui_graph_raw = pickle.load(open(args.data_path + '/netflix/' +  'train_mat','rb'))
        # get user embedding  
        ...
        pickle.dump(augmented_user_init_embedding_final, 
                    open(args.data_path + args.dataset + '/augmented_user_init_embedding_final','wb'))
        ...
        ## BGU note -> load att embedding per user and all his movies
        ...
        for value in augmented_atttribute_embedding_dict.keys():
            ## BGU note -> move on all the movies, value == attribute, i == user
            ## BGU note -> create 1 vectore to each attribute 
        ...
        # TODO -> add here world knowledge extention 
        ## BGU edit

        self.movies_entities_embedding = pickle.load(open('/home/ezradin/LLMRec_update/LLMRec/data/netflix/movies_entities_embedding_finall_bgu','rb'))       
        
        self.model_mm = MM_Model(self.n_users, self.n_items, self.emb_dim, self.weight_size, 
                                 self.mess_dropout, self.image_feats, self.text_feats,
                                 self.user_init_embedding, self.item_attribute_embedding,
                                 self.movies_entities_embedding)      
    ...
    def test(self, users_to_test, is_val):
        ...
        # TODO BGU group edit -> add world knowlage matrix and multiple with ia_embedding
